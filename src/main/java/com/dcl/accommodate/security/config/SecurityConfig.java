...
public class SecurityConfig {
    ...
    private void configureFilterChainWithDefaults(HttpSecurity http, JwtType jwtType) throws Exception {
        log.info("Configuring SecurityFilterChain with JWT Filter for Type: '{}'", jwtType);
        http.csrf(AbstractHttpConfigurer::disable);

        String publicURI = uri.buildPublicPattern("/**");
        log.info("Allowing public access to: '{}'", publicURI);

        http.authorizeHttpRequests(authorize ->
                authorize.requestMatchers(publicURI)
                        .permitAll()
                        .anyRequest().authenticated());

        JwtFilter filter = new JwtFilter(jwtService, jwtType, uri.buildPublicPattern(""));
